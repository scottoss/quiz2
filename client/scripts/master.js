  ...
  if (quiz.status == "question" || quiz.status == "answer") {
    ...
    users.forEach(u => {
      ...
      if (u.token) {
        uHtml = uHtml.replace(
          'add_func=""',
          'href=javascript:addScore("' + u.token + '")'
        );
        uHtml = uHtml.replace(
          'low_func=""',
          'href=javascript:lowerScore("' + u.token + '")'
        );
      } else {
...
function lastQuestion() {
  socket.emit("lastQuestion");
}

function addScore(userToken) {
  socket.emit("addScore", userToken);
}
function lowerScore(userToken) {
  socket.emit("lowerScore", userToken);
}
