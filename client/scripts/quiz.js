...
socket.on("pageUpdate", function(data) {
  ...
  if (quiz != {}) {
    ...
    if (
      ...
      (data.quiz.status == "answer" && quiz.status == "question") ||
      (data.quiz.status == "question" && quiz.status == "answer")
    ) {
...
function setTemplate() {
  if (quiz.status == "waiting")
    $("#page-content").html($("#waiting-template").html());
  if (quiz.status == "score")
    $("#page-content").html($("#score-template").html());
  if (quiz.status == "finished")
    $("#page-content").html($("#finished-template").html());
  ...
  if (quiz.status == "question" || quiz.status == "answer") {
    ...
    if (quiz.type != "choice" && quiz.type != "guess")
      $("#user-answer")
        .parent()
        .removeAttr("hidden");
  }
...
function submitUpdate() {
  if (quiz.type == "guess") {
    socket.emit("userInput", {
      input: $("#guess-answer").val(),
      token: sessionStorage.sessionToken
    });
  } else {
    socket.emit("userInput", {
      input: $("#user-answer").val(),
      token: sessionStorage.sessionToken
    });
  }
}
